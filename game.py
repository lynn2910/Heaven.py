...
from modules.save import saves
...
class Game:
    ...
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        self.map = Map()  # Voir `modules/map.py`
        self.camera = Camera()  # Voir `modules/camera.py`
    ...
    def draw_esc_menu(self, screen: pygame.Surface) -> None:
        ...
        # On dessine le nom de la sauvegarde
        text = fonts.get(20).render(f"Sauvegarde n'{self.save_id}", True, (255, 255, 255))
        screen.blit(text, (screen_width - 50 - text.get_width(), screen_height - 50 - (text.get_height() // 2)))
    ...
    def update_success(self):
        ...
        # Nous vérifions un succès
        if not achievements.is_complete(AchievementsID.THIRD_TIME_RIGHT) and START_GAME_NUMBER >= 3:
            achievements.complete(AchievementsID.THIRD_TIME_RIGHT)
