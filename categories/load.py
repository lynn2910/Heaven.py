...
from modules.achievments import achievements, AchievementsID
...
class LoadGame:
    ...
    def enter(self):
        ...
        self.saves = saves.format(self.saves_per_row)
        ...
        # On modifie le nombre de lignes sélectionnables
        self.MAX_ROWS = len(self.saves[self.actual_page])
        self.actual_row = 0
    ...
    def enter_save(self, engine):
        ...
        """
        # On active le succès
        if not achievements.is_complete(AchievementsID.GET_THE_OLD_ADVENTURE):
            achievements.complete(AchievementsID.GET_THE_OLD_ADVENTURE)
    ...
    def update(self, delta: float):
        ...
        if self.key_cooldown <= 0:
            ...
            if keys[pygame.K_UP] or keys[pygame.K_z]:
                ...
                # On modifie le nombre de lignes sélectionnables
                self.MAX_ROWS = len(self.saves[self.actual_page])
                if self.actual_row >= self.MAX_ROWS:
                    self.actual_row = 0
