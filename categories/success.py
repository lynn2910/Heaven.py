...
class Success:
    ...
    def draw(self, screen: pygame.Surface):
        ...
        if len(self.success) < 1 or (self.actual_page > len(self.success) - 1) or len(
                self.success[self.actual_page]) < 1:
            screen.blit(
                self.no_success,
                (
    ...
    @staticmethod
    def draw_success(screen: pygame.Surface, achievement: Achievement, x: int, y: int):
        """
        ...
        # On dessine le nom
        success_name = fonts.get(25).render(achievement.name, True, color)
        screen.blit(success_name, (x, y + ((height // 2) - success_name.get_height() // 2)))

        # puis la description
        success_desc = fonts.get(25).render(achievement.description, True, color)
        screen.blit(
            success_desc,
            (
                x + 25,
